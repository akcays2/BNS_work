...
\end{align}
%
and the $\tilde{A}_i$ are given in Table~\ref{table:network_params}. %for $i=1,2,3$. %Clearly, the strain and thus the SNR at each of our imaginary interferometers scales as $\langle |Q|^2\rangle^{1/2}_i$, thus
%%
...
\begin{tabular}{c||c|c|l}
 IFO ($i$) & configuration %& $\langle |Q_i|^2\rangle$ 
...
we use the RMS-averaged L1 sensitivity which effectively lowers the solid line by a factor of $2/5$. %, but with RMS-average over all angles 
...
Speculating that the advances in the detection, localization and early-warning algorithms in the next decade will reduce the required early
warning time to less than an hour, we set $T_\text{AW} = 1\,\text{hour}$ and obtain corresponding BNS horizon distances of $D_H=87,613\,$Mpc ($z\approx 0.02,0.127$) for ET-B and ET-C, respectively.
...
such as the black hole mass and spin. The evolution of black hole-neutron star (BH-NS) binaries is a very active branch of numerical relativity
(see Ref. \cite{Shibata:2011jka} for a comprehensive review).
