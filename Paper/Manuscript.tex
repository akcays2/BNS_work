...
(see Ref.~\cite{Faber:2012rw} for a review, and Refs.~\cite{Baiotti:2016qnr,Kyutoku:2017voj, Zappa:2017xba, Dietrich:2018upm} for the latest developments and references therein).
...
%
\be
\dot{E} = \f{32}{5}\f{c^5}{G} \left(\f{G M_c\, \omega}{2c^3}\right)^{10/3}= \f{32}{5}\f{c^5}{G}x^5, \label{eq:Edot2}
\ee
%
where $x = ({G M_c\, \Omega}/{c^3})^{2/3} $ is the dimensionless inverse separation parameter ubiquitous in post- Newtonian theory and $\omega=2\Omega$ is the angular frequency of the quadrupolar GWs. 
...
use design sensitivities of Advanced LIGO (A-LIGO) and ET for $\sqrt{S_n(f)}$.
...
Moreoever, at $f=100\,$Hz, a $m_1=m_2=1.4M_\odot$ neutron-star binary has separation $r\approx 155\,$km, which translates to a dimensionless strength of gravity of $\approx 2.6\%$.
...
See the green dot-dashed curve in Fig.~\ref{fig:figGW170817} for an illustration of these effects.
...
given by Eq.~(\ref{eq:Sch_f_isco}) which translates to the following dominant-mode (quadrupole) GW frequency at the ISCO
\be
...
\hline
\end{tabular}
\parbox{16cm}{\caption{The scaling of the physical quantities of interest in this article in terms the binary masses $m_1, m_2$ restricted to lie between $1.2M_\odot$ and $1.6 M_\odot$. 
%We used Kepler's third law to obtain $f \sim f_K \sim M^{1/2}$.
Value$_{1.4M_\odot}$ represents the value of the quantities in column three evaluated at $m_1=m_2=1.4 M_\odot$ then rescaled to unity.
...
$m_1=m_2=1.4 M_\odot$ is more likely to underestimate $T_\text{AW}$ than overestimate it thus erring on the side of caution.
...
\ee
%
where we introduced the dimensionless Kerr spin parameter $\chi\equiv {J}/({G m^2})$ which, for a neutron star (NS) with spin $J$ and mass $m$, can be written as
%
\be
...
the fastest observed NS spin is $\lesssim 0.04$ \cite{FastestNSspin}.
Therefore, at $f= 100\,$Hz, the first term on the right-hand-side of Eq.~(\ref{eq:spin_orbit}) gives $\approx 4\times 10^{-4}$. 
The other spin effect, the spin-spin coupling, contributes at $\ord(v^4/c^4)\times\dot{E}$ hence it is $\lesssim \dot{E}_\text{SO}$.
...
is the appropriate power. The standard treatment adopts a Friedmann-Le Ma\^{i}tre-Robertson-Walker cosmology with scale factor $a(t)$ and curvature $k=\pm 1, \text{ or } 0$ \cite{cosm_book}.
...
\end{align}
%
We see that all three equations are rescaled by $(1+z)^{\pm5/3}$. 
Now, looking at Eqs.~(\ref{eq:fdot}, \ref{eq:tau_insp}, \ref{eq:h_c}) we spot a common factor of $M_c^{\pm5/3}$ in all of them.
...
In short, the effects of the cosmological redshifting can be accounted for by the transformation $M_c\rightarrow \mathcal{M}_c$.
...
Therefore, we neglect the effects of cosmological redshift 
when considering sources for the ALV network.
On the other hand, for Einstein Telescope, we go out to $D=1\,\text{Gpc}\implies z\approx 0.2$
which reduces $\tau_\text{insp}$ by $\approx 25\%$, thus we shall include the redshift effects in our computations of advance warning times and SNRs
...
swept the ALV network's frequency band from $\approx 30\,$Hz to $\approx 2000\,$Hz, accumulating a total network SNR of
...
For $S_\text{n}(f)$ we use actual [noise-subtracted] LIGO-Livingstone (L1) data for GW170817 available at LIGO's website \cite{LIGO_L1_data}.
...
The breakdown of the leading-order $\sqrt{f}f^{-7/6}=f^{-2/3}$ behavior of
the inspiral around $f\sim100\,$Hz due to tidal effects is clear.
...
\section{Results: Advance warning times for future binary neutron star inspirals}\label{sec:results}
 ...
 Our definition of advance warning is the inspiral time to the merger from a certain threshold instant $\bar{t}$ at which point the network ``agrees'' that there is
 an event statistically significant enough to issue warnings to electromagnetic telescopes.
...
at luminosity distances of $100,200,400,1000\,$Mpc corresponding to cosmological redshifts of $z\approx 0.0222,0.0437, 0.085, 0.198$, respectively.
...
Sweeping across these are the RMS-averaged strains of four BNS inspirals at $D=100,200,400,1000\,$Mpc represented by solid (black), dotted (blue), dashed (green),
...
In actuality, the inspirals continue beyond the test-mass $f_\text{ISCO}$, which we illustrate using the faded lines in the plot (drawn to 5\,kHz for aesthetic reasons). The plunge to merger is expected to occur at $f\approx 2\,$ kHz \cite{Marronetti:2003hx}.
...
Table~\ref{table:ET} summarizes our results for both ET-B and ET-C where we chose to only display the 3.5PN-accurate inspiral time for $T_\text{AW}$,
...
We now investigate the possibility of witnessing the tidal distruption of a neutron star by a black hole before the merger.
...
where $R_\text{NS}$ is the radius of the NS and $M$ denotes mass. 
...
\begin{enumerate}[(i)]
 ...
 \item Low black hole mass. In the test-mass case, $r_\text{ISCO} \propto M_\text{BH}$, so the Newtonian tidal force at the ISCO scales as $M_\text{BH}/r_\text{ISCO}^3\propto M_\text{BH}^{-2}$. 
 ...
 \item High black hole spins. $r_\text{ISCO}$ is much smaller in the case of a prograde NS orbit around a spinning Kerr black 
 hole\footnote{Prograde orbits have $\hat{\mathbf{L}}\cdot\mathbf{J}=|\mathbf{J}|$ whereas for retrograde orbits $\hat{\mathbf{L}}\cdot\mathbf{J}=-|\mathbf{J}|$,
 where $\mathbf{J}$ is the spin vector of the Kerr black hole.}.
 For instance, for the maximum dimensionless BH spin value of $\chi\equiv c J_\text{BH}/(G M^2_\text{BH})=1$, $r_\text{ISCO}$ for prograde orbits is 1/6th of the Schwarzschild value $6GM_\text{BH}/c^2$ whereas for $\chi=1$ retrograde orbits, $r_\text{ISCO}$ is 3/2 of the Schwarzschild value.
...
Although the results from each numerical study of BH-NS mergers may somewhat vary, it is clear is that the BH-NS binary must have a high-spin, low-mass black hole for tidal disruption to happen. 
...
they are spun up by accretion from their secondary (that will later become a NS) in binary systems, thus inevitably gaining mass.
...
There is additional motivation to choose low BH mass as 
 ...
 Setting $M_\text{NS}=1.4 M_\odot$ as before gives $Q=5/1.4  \simeq 3.571$. $c_\text{NS}$ is determined by the radius of the NS constrained to be in the $9.9 - 11.2\,$km range \cite{Ozel:2016oaf}
...
If we pick the more pessimistic $c_\text{NS} = 0.15$ bound then we must have $Q\lesssim 5$ and $\chi\gtrsim 0.5$ (prograde) for TD to occur \cite{Ferrari:2008nr}
and $Q< 3$ in the case of $\chi=0$ \cite{Kyutoku:2011vz}.
...
Given that the A-LIGO O1 merger rate for these systems is $ R < 3600\,$Gpc$^{-3}$yr$^{-1}$ \cite{Abbott:2016ymx},
there could be $\approx 30$ advance warnings of such mergers per year. %within ET-C's horizon distance of $\approx 200\,$Mpc. %corresponding roughly to $T_\text{AW}\approx1\,$hour.
...
\acknowledgements 
SA thanks Conor O'Toole for endless feedback, and Niels Warburton and L\'{a}na Salmon for a thorough perusal of this article. 
