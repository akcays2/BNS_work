...
Using current BNS event rates within this horizon distance, we show that this translates to forecasts of $\gtrsim \ord(10^2)$ GRBs in the 2030s. %with Einstein C. %with the C configuration of Einstein Telescope.
...
Moreover, both the gravitational wave (GW) and EM observations were consistent with a source luminosity distance of $\sim 40\,$Mpc
...
between the merger and the instant of detection defined in terms of a certain threshold signal-to-noise ratio.
...
numerical treatment formulated in terms of general relativistic, magnetohydrodynamic codes 
...
Despite its failures, our quadrupole radiation driven Newtonian
...
Labelling the masses by $m_1 \le m_2$, we have, for a binary in a circular orbit with Keplerian angular frequency $\Omega$ and separation $r$,
%
\be
...
As $x\propto v^2/c^2$, it is used to keep track of post-Newtonian orders and we can see from in Eq.~(\ref{eq:Edot2}) that $\dot{E}\propto x^5$. 
...
Using $\tau$ we can now define an inspiral time at a given GW frequency
%
\be
...
it can be shown that there exist only two physical degrees of freedom which manifest themselves as two independent polarization amplitudes $h_+$ and $h_\times$. %when interacting with masses away from the source.
...
Higher-order contributions can be added in terms a post-Newtonian series, which make up to $\lesssim 2\%$ of the total phase.
...
If we were to conceive of a ``toy'' detector made up of a circular configuration of test-masses lying in the $x$-$y$ plane, a GW propagating along the $z$ direction would stretch/compress the circular arrangement in a $+$ and $\times$ pattern 
with a $90^\circ$ phase difference.
...
The last column shows how loud the GW strain is at a given IFO due source sky position and IFO orientation. 
...
As we can not a priori know the source sky position or orientation we will model the response of ET using the RMS-averaged SNR %(\ref{eq:ET_SNRsq})
%
\be
...
however, there is no simple expression for it akin to Eq.~(\ref{eq:f_isco}) and its extraction from numerical simulations is a rather involved procedure.
...
Including the error bars, the known range for neutron star masses runs from $\approx 0.5 M_\odot$ to $ \approx 3 M_\odot$ with the actual values restricted
between $\approx 1 M_\odot$ and $\approx 2 M_\odot$ \cite{Ozel_Freire} (one exception being J1748-2021B with mass $2.74^{+0.21}_{-0.21} M_\odot $ \cite{Freire_et_al_2007}).
...
$m_1=m_2=1.4 M_\odot$ is more likely to underestimate $T_\text{AW}$ than overestimate it which is something we can live with.
...
we obtain $e\approx 6\times 10^{-5}$. Thus our setting $e=0$ is very reasonable.
%
%
...
Given that we might at best have a couple of minutes of warning time for a once-a-decade event, we do not expect to be able forecast GRBs in the Advanced LIGO-Virgo era. 
...
low-mass BH in a binary system\footnote{We thank Dr. Morgan Fraser
