...
\todo{1) See end of Sec. IV on page 13, 2) Finish new Sec VIII on BH-BH inspirals, 3) page 19. footnote, 4) fix TAble VI}
\end{abstract}
\maketitle
...
inspiral and merger of two neutron stars, causing the prompt short-hard gamma-ray burst GRB170817A. %as 
...
%(projected SNR of GW150914 at ET is 800, at CE 2400 vs. ALV 25)
...
(b) study the cosmological evolution of the stellar mass black holes (see Sec.~2 of Ref.~\cite{ET_doc} for an excellent summary and further details).
...
early enough to witness gamma-ray bursts as they happen? In other words, can we forecast GRBs using GW detections?
...
%we are mostly concerned with how early the GW networks can detect the inspiral before the merger.
...
We then reapply our framework to binary black hole inspirals in Sec.~\ref{sec:BH_BH}.
...
gives the dominant contribution to the power emitted %in gravitational radiation
...
%\simeq 20.37\,\text{hours}\, \left(\f{T_K}{1\,\text{s}}\right)^{8/3} \left(\f{2.8M_\odot}{M}\right)^{2/3}\,\left(\f{0.7M_\odot}{\mu}\right)
\label{eq:tau_of_r},
\ee
%
...
This fact is most easily demonstrated in the so-called transverse-traceless gauge (cf. Sec.~2.1 of Ref.~\cite{Flanagan:2005yc}),
 but is a gauge-invariant consequence of linearized vacuum solutions. %when interacting with masses away from the source.

%In a suitable gauge, such as the commonly used transverse-traceless gauge, 
%In the ``lab'' frame, usually taken to be the transverse-traceless gauge, 
...
The second IFO is near-optimally oriented/positioned so we assign it a quality
...
%our particular choice to represent the sensitivities of the near-future ALV network.
...
\ee
%for $M \equiv m_1+m_2 = 2.8 M_\odot$. 
...
in $f_\text{ISCO}$ should not matter as it affects the advance warning times by less than a second (see above).
...
Let us apply this to the Hulse-Taylor binary pulsar \cite{Hulse:1974eb,Weisberg:2010zz} to show that the BNS orbits circularize by the time they enter the interferometers' bandwidth (ca. $1-10\,$Hz).
Letting $f =2f_K= 1\,\text{Hz}$, i.e., 
$a \simeq 3352\,$km and using the currently observed values $a_i \approx 1.95\times 10^6\,$km and $e_i \approx 0.617$,
we obtain $e\approx 6\times 10^{-5}$ at 1\,Hz. Thus our setting $e=0$ is very reasonable\footnote{
...
The ratio in Eq.~(\ref{eq:Oct_quad_ratio}) is further supressed by $(m_2-m_1)^2/M^2$ which, for our idealized equal-mass system, 
...
the fastest observed NS spin in a BNS system is $\lesssim 0.04-0.05$ \cite{0264-9381-26-7-073001, Stovall:2018ouw}.
Therefore, at $f= 100\,$Hz, the first term on the right-hand-side of Eq.~(\ref{eq:spin_orbit}) gives $\approx 4\times 10^{-4}$. 
On the other hand, the spin-spin coupling contributes at $\ord(v^4/c^4)\times\dot{E}$ hence $\lesssim \dot{E}_\text{SO}$.
...
Accordingly, the inspiral evolves faster in the lab frame %because $\dot{f}$ has increased and the cut-off frequency has decreased.
...
%
\item {\it Changing the detection threshold from $\bar{\rho}_\text{tot}=15$.}\\
\todo{ADD SNR=12 and 20 computations for ET-C}
$\bar\rho$ is defined in Sec. V
%
%
...
%In fact, detailed follow-up analyses () have recently restricted the above-values to
%
...
%
%\footnote{Although the equations of motion for circular orbits is now known up to 4PN \cite{4PN_EoM_paper}, we are not aware of any publications
%explicitly showing the 4PN contributions to $\tau_\text{insp}$ and $\mathcal{N}_\text{cyc}$. Be that as it may, we suspect that a few PN experts have already obtained these.} 
%of $\tau_\text{insp}\approx 60$\,seconds and $\mathcal{N}_\text{cyc} \approx 3000$ which we can obtain if we use $f_i \approx 28-29\,$Hz.
...
Therefore, they will not start accumulating significant SNR until $f\approx 10\,$Hz, thus offering us gains of mere minutes compared to $T_\text{AW}$ in Table~\ref{table:LIGO2020}. For this reason, we move on to 2030s: the 
...
Our values for $T_\text{AW}$ for 200 and 400\,Mpc are roughly consistent
...
inside a radius of $D\approx 1000\,$Mpc to within $ 10\,\text{deg}^2$ \cite{Mills:2017urp}, but mostly after the merger.
%and more than half the sources within $D\lesssim 400\,$Mpc to $\approx 1\,\text{deg}^2$ . 
On the other hand, ET with CE together will localize 
...
since CE will be sensitive enough to start accumulating SNR at $f\approx 5\,$Hz from BNSs within 400\,Mpc corresponding to $T_\text{AW}\gtrsim 1.5\,$hours.
Even without the help of a second detector, it will be possible
to localize $\sim 5\%$ of these transients to within $10\,$deg$^2$ using
the Earth's rotation because these nearby sources will spend hours in the ET-C band \cite{Zhao:2017cbb}.
This percentage amounts to $\sim 5$ events per year.
%Additionally, these localizations are mostly post-merger as neither Voyager,
Erring on the side of optimism, we conclude that the 2030s hold in store real-time observations of dozens to hundreds of gamma-ray bursts and the subsequent kilonovae
\footnote{As the ET construction/commissioning timeline is currently uncertain, we can only speculate that ET-C will be operational in mid 2030s,
which is roughly in line with the timeline of the milli-Hertz space interferometer LISA \cite{Audley:2017drz} and may be five to ten years ahead of CE \todo{citation}}.
...
%bottomrule
\end{tabular}
\caption{\todo{FIX these numbers}Same as Table~\ref{table:ET}, but now for a $5 M_\odot- 1.4 M_\odot$ black hole-neutron star binary.
...
there could be $\approx 30$ such mergers per year within ET-C's range of $\approx 200\,$Mpc. %corresponding roughly to $T_\text{AW}\approx1\,$hour.
...
%\ee
%
\section{Bnary black hole inspirals}\label{sec:BH_BH}
We briefly repeat our computations for binary black hole (BBH) inspirals.
As heavier systems inspiral too fast, we limit ourselves
to $5M_\odot-5M_\odot$ BBHs and only consider ET-C.
For these systems, we find that $T_\text{AW} \lesssim 1\,$hour for $D\gtrsim 190\,$Mpc, but $\bar{\rho}_F$ will exceed 500. As such lightweight black holes
are expected to be rare and given the BBH merger rate inferred from the previous LIGO detections \todo{FInd the event rate}, we expect no forecasts
for these events. Nonetheless, ET will detect hundreds to thousands of BBH mergers.
