...
\begin{tabular}{c|c|c|c}
 IFO$_i$ & GW strain %& $\langle |Q_i|^2\rangle$ 
...
We artificially end the inspirals at the Schwarzschild ISCO of the BNS with coordinate radius $r_\text{ISCO} = 6M$ and angular frequency $\Omega_\text{ISCO}$ %= (M/r^3_\text{ISCO})^{1/2}$ in natural units, 
...
$\tilde{H}$ & $M_c^{5/6} $ & $(m_1 m_2)(m_1+m_2)^{3/2} $&\quad& $ 0.7834\le 0.891 \le 0.996 $ \\
$f_\text{ISCO}$ & $M^{-1} $ & $(m_1+m_2)^{-1} $&\quad& $0.4375\le 0.5\le 0.583 $\\
\hline
\end{tabular}
...
Moreoever, it seems that the variation in mass tends to lengthen $\tau_\text{insp}$ more than shorten it thus our setting
$m_1=m_2=1.4 M_\odot$ is more likely to underestimate $T_\text{AW}$ than overestimate it which is something we can live with.
So we set the NS masses to equal $1.4 M_\odot$ henceforth.
%
...
Thus, NS spins do not significantly affect our estimation of advance warning times. %$T_\text{AW}$.
...
Although the results from each numerical study may somewhat vary what is clear is that we must have high spin and low black hole mass. We are thus led to set $M_\text{BH} =5 M_\odot$. 
...
There is additional motivation to choose low BH mass as 
 $\tau_\text{insp} \propto M_c^{-5/3} = (1+Q)^{1/3}/Q$ for $Q\equiv M_\text{BH}/M_text{NS} > 1$, which scales as $Q^{-2/3}$ as $Q\to\infty$ , i.e., heavier binaries inspiral faster.
 We once again set $M_\text{NS}=1.4 M_\odot$ giving $Q=5/1.4  \simeq 3.571$. $c_\text{NS}$ is determined by the radius of the NS constrained in the $9.9 - 11.2\,$km range \cite{Ozel:2016oaf}
