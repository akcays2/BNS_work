...
\affiliation{Friedrich-Schiller-Universit{\"a}t Jena}
%\date{\today}
...
Throughout the text, $t$ denotes observer/detector time and $f$ denotes the GW frequency of the dominant quadrupole mode, which is twice the orbital (Keplerian) frequency $f_K$. 
...
mass of a neutron star due to Chandrasekhar limit \cite{Chandra}.
...
(the details of the Fourier transformation can be found in Sec.~4.5 of Ref.~\cite{Maggiore} or in \cite{Moore:2014lga}).
...
but varying response to GWs due to IFO orientation and source polarization and sky position. This response is encoded in the quality factor $|Q_i|$.
