...
\title[Forecasting GRBs with GWs] % (optional, use only with long paper titles)
...
%\renewcommand{\c}{\cos}  \c{c} gives cecedille
\renewcommand{\t}{\theta}
\renewcommand{\b}{\bar}
\newcommand{\f}{\frac}
\newcommand{\bt}{\beta}
...
\begin{document}
{
  \usebackgroundtemplate{\includegraphics[width=1\paperwidth,height=\paperheight]{figs/kilonova.jpg}}
  % image credit [University of Warwick/Mark Garlick]
  \begin{frame}
  \vspace{-2cm}
    \titlepage
  \end{frame}
} % these brackets are needed to insure that the background image is used ONLY in the title slide
...
\begin{frame}{Motivation: Multi-messenger Astronomy}
  \begin{itemize}
    ...
    \onslide<2->\item \red{GW170817}-\bl{GRB170817A}-\textcolor{brown}{AT2017gfo}: the ``event'' \\
    ...
    \onslide<3->\item A simple question: \\
...
\begin{small}
    \begin{itemize}
      \item Discovered by Vela satellites in \red{1967}
      \onslide<2->\item Non-thermal spectrum: $10-10^4\,$\red{keV}
      \item Highly energetic $10^{48}-10^{\red{52}}\,$ergs 
      \onslide<3->{\item \bl{1000s} detected, $\sim 1$ per day}
      \onslide<4->\item Far away: $z\approx 1 \sim 8\,$billion years! %$9.4\sim 13\,$billion light years!
      \onslide<5->\item Afterglow from X-rays to radio \\
      over timescales of hours to months
      \onslide<6->\item \bl{Two types}: \red{short} and long
      \onslide<7->{\item Short GRBs:\\ 
      mergers of binary neutron stars (1989)\\
...
\begin{frame}{Neutron Stars}{Most extreme stars in the universe}  
  \begin{small}
    \begin{itemize}
      ...
      \item Mass: $\sim $ \red{1} to \red{3} solar masses ($M_\odot$)
      \item Very \red{compact}: radius $\sim 9$ to 15\,km
      \onslide<2->\item High spins: up to 700 times/second
      \onslide<3->\item Extremely \bl{dense}: teaspoon $\sim 10^9\,$tons
      \onslide<3->{\item Large magnetic fields: $10^4$ to $10^{\red{11}}\,$T}
      \onslide<4->\item Hot: surface temperature $10^5 - 10^6\,$K
      \onslide<5->\item They can come in \red{pairs} (binary) \\
      \onslide<6->Orbit \red{decays} over time due\\
      $\Longrightarrow$ \underline{Gravitational wave emission}!\\
      Observations \red{match} theoretical \\predictions very well
      %Equation of state \\
  ...
  \begin{tikzpicture}[overlay,remember picture]
	...
	\node[anchor=center,scale=1,opacity=1] at ([shift={(5.4cm,2.9cm)}]current page.center) {\textcolor{white}{SXS}};
      \onslide<2>\node (img1 )[anchor=center,scale=0.8,opacity=1] at ([shift={(3.8cm,0.9cm)}]current page.center) {\includegraphics[height=6cm]{figs/Millisecond_NS2.png}}; 
      \node[anchor=center,scale=0.5,opacity=1] at ([shift={(4.8cm,3.2cm)}]current page.center) {Class.Quant.Grav. 30 (2013) 224003};
      ...
      % Use Fig 3 of http://science.sciencemag.org/content/304/5670/536/tab-figures-data ?
      \node[anchor=center,scale=0.6,opacity=1] at ([shift={(4.6cm,-1.5cm)}]current page.center) {Robert Schulze};
      \onslide<4>\node (img2 )[anchor=center,scale=0.8,opacity=1] at ([shift={(3.6cm,0.0cm)}]current page.center) {\includegraphics[height=5cm]{figs/NS_temp.jpg}}; 
      %
      \node[anchor=center,scale=0.6,opacity=1] at ([shift={(4.6cm,2.2cm)}]current page.center) {Science 304, 5670, pp. 536-542};
       \onslide<5->\node (img3 )[anchor=center,scale=0.8,opacity=1] at ([shift={(3.cm,-2.8cm)}]current page.center){\includegraphics[height=3.8cm]{figs/HT_pulsar.jpg}};
       \onslide<6>\node (img3 )[anchor=center,scale=0.8,opacity=1] at ([shift={(3.2cm,0.9cm)}]current page.center){\includegraphics[height=5cm]{figs/HT_decay.jpg}}; 
       \node[anchor=center,scale=0.5,opacity=0.7] at ([shift={(2.5cm,-0.5cm)}]current page.center) {ApJ 829: 55 (2016)};
       %{\includegraphics[height=5cm]{figs/NS_EOS_Lattimer2012.png}}; 
...
\begin{center}
  \begin{tikzpicture}%[overlay,remember picture]
	      ...
	      \onslide<2-4>{\draw[red,thick] (1.7,0.04) rectangle (3.1,0.27);}
  \end{tikzpicture}
  \\
  \vspace{5mm}
  \onslide<2->{Perturbations of spacetime with speed $=c$, sourced by accelerating masses.}
\end{center}
    %      
  \onslide<3->
  \[\text{Wave equation: } \boxed{\Box \bar{h}_{\mu\nu}= 0} \ \ \Longrightarrow
  \alert{\text{Plane-waves: }}  \bar{h}_{\mu\nu} = \Re\left[A_{\mu\nu}\, \text{e}^{-i\omega (t-z/\red{c} )}\right]
  \]
  %
 \onslide<4-> $A_{\mu\nu} $: polarization tensor, \\
 $\Longrightarrow$ \emph{only} \red{2} polarizations (\bl{transverse}):
  $\begin{array}{l} \w{A_{xx}=-A_{yy} = h_+}  \\ \w{A_{xy}=\ \ A_{yx} = h_\times} \\ \end{array}$
 \end{frame}
...
\begin{center}
  \begin{tikzpicture}%[overlay,remember picture]
	      \onslide<1>\node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[height=3.5cm]{figs/white_box.png}};
	      %\onslide<2-4>{\draw[red,thick] (1.7,0.04) rectangle (3.1,0.27);}
  \end{tikzpicture}
  \\
  \vspace{5mm}
  {Perturbations of spacetime with speed $=c$, sourced by accelerating masses.}
\end{center}
  %        
  %
  \[\text{Wave equation: } \boxed{\Box \bar{h}_{\mu\nu}= 0} \ \ \Longrightarrow
 ...
 $A_{\mu\nu} $: polarization tensor, \\
 $\Longrightarrow$ \emph{only} \red{2} polarizations (\bl{transverse}):
  $\begin{array}{l}A_{xx}=-A_{yy} = \red{h_+}  \\ A_{xy}=\ \ A_{yx} = \red{h_\times}  \\ \end{array}$
 \begin{tikzpicture}[overlay,remember picture]
	      \node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(-3.5cm,1.5cm)}]current page.center){\animategraphics[loop,controls,height=2.6cm]{12}{GWs_plus-}{0}{20}};
	      \node (img1 )[anchor=center,scale=1.,opacity=1] at ([shift={(3.5cm,1.5cm)}]current page.center) {\animategraphics[loop,controls,height=2.6cm]{12}{GWs_cross-}{0}{20}};
\end{tikzpicture}
\end{frame}
...
%  Two of the same slide next for the right transition effects %%%%%%%%%%%%%%%%%%%%%%%
 ...
 \begin{frame}{Making gravitational waves}{non-spherical, accelerated motion}
  \alert{Inspirals}, bumps on neutron stars, supernovae, the Big Bang. \\%  (BICEP 2).\\
...
\begin{frame}{Making gravitational waves}{non-spherical, accelerated motion}
  ...
  \vspace{2mm}
  \begin{small}$ \quad \bar{Q}_{ij} = Q_{ij}-\frac{1}{3}\delta_{ij}Q^k_k, \quad Q^{ij} = \int \rho(t,{\mathbf x})x^i x^j d^3 x $ \end{small} \end{center}
  {\bf Binary systems:} 2 points masses in a circular orbit $M=m_1+m_2$
  \begin{tikzpicture}[overlay,remember picture]
    ...
    \onslide<3->\node (t2 )[anchor=center,scale=1,opacity=1] at ([shift={(2.25cm,-2.2cm)}]current page.center) {$ Q^{ij} =  \mu\, x^i x^j, \qquad \boxed{\dot{E} = \f{32}{5}\f{G\mu^2}{c^5}r^4 \Omega^6}$};
    %\onslide<4->\node (t3 )[anchor=center,scale=1,opacity=1] at ([shift={(2.5cm,-3.2cm)}]current page.center) {Enhancement due to eccentricity \bl{$e$}};
...
\begin{frame}{Compact Binary Inspirals}
  ...
  \onslide<4->
      \begin{tikzpicture}[overlay,remember picture]
      \node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(3.9cm,-3.25cm)}]current page.center) {\includegraphics[height=3.5cm]{figs/R_of_Tau.pdf}};
      \onslide<5->\node[draw, red,thick,minimum width=9.5cm,minimum height=1.3cm] (b) at (4.4,3.35){};
...
\end{frame}
 ...
 \begin{frame}{GW Detectors: \red{Interferometers}}{Geodesic deviation}
	...
	\node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(3.8cm,-2.8cm)}]current page.center) {\includegraphics[height=2.39cm]{figs/white_box.png}};
      \end{tikzpicture}
          \begin{small}
      \begin{align*}
...
%
  ...
  \onslide<2->
  {
    ...
    \begin{footnotesize} 
    \[
    \rho_{\text{tot},\bigtriangleup} = \left[\sum_{A=1}^3 \left(\rho^A\right)^2\right]^{1/2}  =  A \rho_c \,\langle|Q(\theta,\phi,\psi,\iota)|^2\rangle^{1/2}_\bigtriangleup %\int_0^{f_\text{ISCO}} d f\, \f{f^{-7/3}}{S_n(f)} \,
    \]
    \end{footnotesize}
    \\
  }
  ...
  &=\f{6}{5}A\, h_0 \rho_c \\%\left[\int_0^{f_\text{ISCO}} d f\, \f{f^{-7/3}}{S_n(f)}\right]^{1/2} \\
  \end{align*}
  \end{footnotesize}
  \\
  ...
  %
  \begin{tikzpicture}[overlay,remember picture]
  \node[draw, red,thick,minimum width=3.cm,minimum height=0.9cm] (b) at (-3.2,0.9){};
  \end{tikzpicture}
  %
  % 
\end{frame}
...
\begin{tikzpicture}[overlay,remember picture]
    ...
    {\begin{footnotesize} 
      $\bar\rho_{i} = \left\{28.7, \ 14.3, \ 6.00 \right\}$    
    \end{footnotesize}}; 
    \onslide<4->\node (t4 )[anchor=center,scale=1,opacity=1] at ([shift={(4cm,-2.6cm)}]current page.center) 
    {\begin{footnotesize} 
      $\bar\rho_\text{LHV} = \{ 26.4, 18.8 , 2.0\} $    
    \end{footnotesize}};
    \onslide<3->\node (t5 )[anchor=center,scale=1,opacity=1] at ([shift={(4cm,-1.7cm)}]current page.center) 
    {\begin{footnotesize} 
      $\rho_\text{tot} = 32.0 $    
    \end{footnotesize}};
        \onslide<4->\node (t6 )[anchor=center,scale=1,opacity=1] at ([shift={(4cm,-3.1cm)}]current page.center) 
    {\begin{footnotesize} 
      $\rho_\text{tot} = 32.4 $    
    \end{footnotesize}};
...
\begin{small}
  \[
    \bar\rho^2_\text{tot}= \left[\rho_h(f_0^h,\bar{f})\right]^2+\left[\rho_m(f_0^m,\bar{f})\right]^2+\left[\rho_l(f_0^l,\bar{f})\right]^2=\red{15}^2
  \]
\end{small}  
...
\begin{frame}{GW Astronomy in the 2020s}{Advanced LIGO-Virgo}
  ...
  \begin{tikzpicture}[overlay,remember picture]
    \onslide<1->\node[draw, red,thick,minimum width=2.8cm,minimum height=1.0cm] (b) at (-0.7,4.){};
    \onslide<3->\node[draw, red,thick,minimum width=1.1cm,minimum height=1.2cm] (b) at (1.6,1.35){};
     %\onslide<4->\node[draw, blue,thick,minimum width=1.1cm,minimum height=1.2cm] (b) at (3.2,1.35){};
  \end{tikzpicture}
  \\
  \vspace{2mm}
  \onslide<4->
    ...
    \end{footnotesize}
      \begin{tikzpicture}[overlay,remember picture]
         \onslide<4->\node[draw, blue,thick,minimum width=1.1cm,minimum height=1.2cm] (b) at (9.7,3.55){};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{GW Astronomy in the 2030s}{The era of Einstein Telescope}
  \vspace{-1mm}
  \begin{itemize}
    \item[] One detector, \red{three V}-IFOs, $60^\circ$ 10\,km arms.
    \item[] Underground, cryogenic, \bl{xylophone} (high, low-frequency modes)
    \uncover<2->{
    \item[] Recall: \red{RMS}-average over all four angles
    \item[] Cosmological range $\Rightarrow$ \red{red}-shifted strain and SNR}
  \end{itemize}
  \onslide<2->
    \begin{footnotesize}
    \begin{align*} 
  ...
  \onslide<3->
    Consider BNS at
    \begin{footnotesize}
      $$ \bl{D}=\{100,200, 400, 600\}\,\text{Mpc} \Longrightarrow \red{z}=\{0.022, 0.044, 0.085, 0.12\}$$
      $$ \left(\text{Using } \Omega_\Lambda = 0.6911, \Omega_m = 0.3089, H_0 = 67.74\,\text{km}\,\text{s}^{-1}\,\text{Mpc}^{-1}\right)\qquad  $$
    \end{footnotesize}
  \onslide<4->
  {{\bf Note}: for binary black holes, ET's range $>\,1$Gpc, but inspirals too quick.}
\end{frame}

\begin{frame}{The era of Einstein Telescope}{BNS detections}
 Same detection criterion:\qquad $\bar{\rho}_\text{ET}(f_0, \bar{f}_\text{ET}) =15$\\
 \vspace{1mm}
 \red{Two} ET configurations:\qquad \textcolor{brown}{B} (high $f$), \red{C} (low $f$)
 \begin{tikzpicture}[overlay,remember picture]
      \onslide<1->\node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(0cm,-1.5cm)}]current page.center) {\includegraphics[height=6cm]{../Figures/ET_strains_redshifted_pres.pdf}};
      \onslide<2->\node (img1 )[anchor=center,scale=1,opacity=1] at ([shift={(0cm,-1.5cm)}]current page.center) {\includegraphics[height=6cm]{../Figures/ET_B_SNR_int.pdf}};
      \onslide<3->\node (img1 )[anchor=center,scale=1,opacity=1] at ([shift={(0cm,-1.5cm)}]current page.center) {\includegraphics[height=6cm]{../Figures/ET_C_SNR_int.pdf}};
\end{tikzpicture}      
\end{frame}
...
\begin{frame}{The era of Einstein Telescope}{GRB \red{Forecast} Prospects } 
  ...
  %
  \vspace{-3mm}
  \onslide<2->Very \red{high SNRs} $\Rightarrow \ord(\rho_F^{\red{-1}})$-precision parameter estimation afterward \\
  \vspace{2mm}
  \onslide<3->\bl{ET-B} $\Longrightarrow $ forecasts not likely \\
  \vspace{2mm}
  \onslide<4->\red{ET-C} $\Longrightarrow T_\text{AW} \ge 1\,$hour within 600\,Mpc\\
  \vspace{1mm}
  \textcolor{white}{ET-C} $\Longrightarrow \red{\ord(100)}$ {\bf BNS forecasts} yearly! $\Rightarrow $ 1 per day!\\
  \vspace{1mm}
  \onslide<5->\textcolor{white}{ET-C} $\Longrightarrow \red{\ord(10)}$ with \red{3\,hours} of warning \\
  \vspace{1mm}
  \onslide<6->\textcolor{white}{ET-C} $\Longrightarrow \red{\ord(2)}$ with \red{5\,hours} of warning
  \begin{tikzpicture}[overlay,remember picture]
      \onslide<3>\node[draw, blue,thick,minimum width=2.cm,minimum height=1.6cm] (b) at (-0.9,5.3){};
      \onslide<4->\node[draw, red,thick,minimum width=1.8cm,minimum height=1.6cm] (b) at (3.6,5.3){};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{The era of Einstein Telescope}{Source localization before the merger} 
  ...
  \end{itemize}
  \onslide<3->So, within 1\,Gpc, ET {\bf alone} will localize
  \begin{itemize}
  %\item 20\% of the sources to within 100\,deg$^2$
  \item 2\% to within 20\,deg$^2$
  \item 0.5\% to within 10\,deg$^2$ 
  \onslide<4->\item[]$\Longrightarrow \red{5}$ to \red{10} sources within 400\,Mpc
  \end{itemize}
  \onslide<5->10\,deg$^2$: LSST's field of view\\
  \vspace{2mm}
  \onslide<6-> Numbers improve with \bl{Cosmic Explorer} ($f\gtrsim \red{5}\,$Hz)
  %
  \begin{tikzpicture}[overlay,remember picture]
	\onslide<2->\node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(4.2cm,2.25cm)}]current page.center) {\includegraphics[height=3cm]{figs/Earth_rot.png}};
	\onslide<5->\node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(4.3cm,-2.25cm)}]current page.center) {\includegraphics[height=3.9cm]{figs/LSST.jpg}};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Black hole - neutron star binaries}{Forecasting \red{tidal disruption} events}
  ...
  %
  \begin{tikzpicture}[overlay,remember picture]
	  \onslide<1->\node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(5.6cm,2.25cm)}]current page.center) {\includegraphics[height=2.8cm]{figs/BH_NS_1.png}};
	  \node[anchor=center,scale=0.5,opacity=1] at ([shift={(5cm,0.7cm)}]current page.center) {Phys.Rev. D92 (2015) 044028};
  \end{tikzpicture}
\end{frame}
...
{\bf Summary}
\begin{itemize}
 \item \bl{Quadrupole} + \bl{Newtonian} evolution = inspiral time $\tau_\text{insp}\propto \red{f^{-8/3}}$
 \item Detector sensitivity + GW strain = advance warning time $T_\text{AW}$
 \onslide<2->
 \item Advanced LIGO-Virgo will detect \bl{many} BNS, but NOT forecast.
 \onslide<3->
 \item Einstein Telescope will \bl{detect} $\bl{\ord(1000)}$ of BNS out to 1\,Gpc .
 \item Einstein-C will \red{forecast $\ord(100)$ BNS} with $T_\text{AW}>1\,$hour.
 \item 5 to 10 BNS inspirals will be \red{localized} to $\red{10\,\text{deg}^2}$.
 \item Additionally forecast $< \ord(10)$ tidal disruption events.
 \end{itemize}
 \onslide<4->{
  {\bf Outlook}
  \begin{itemize}
    \item Forecast/localize $\Rightarrow$ Mobilize EM telescopes \\
  \hspace{-5.6mm}$\Rightarrow$ \red{Observe} the merger/kilonova from  $\gamma$-rays to radio
  \item Pre-, post-merger image comparisons with LSST
 }
 \onslide<5->
 \item Contaminants? Type Ia supernovae, AGN, CV
\end{itemize}
\begin{tikzpicture}[overlay,remember picture]
	  \onslide<5->\node (img0 )[anchor=center,scale=1,opacity=1] at ([shift={(-2.6cm,-3.8cm)}]current page.center) {\includegraphics[height=1.2cm]{figs/AAletter.png}};
	  \onslide<5->\node (img1 )[anchor=center,scale=1,opacity=1] at ([shift={(4.6cm,-3.cm)}]current page.center) {\includegraphics[height=2.7cm]{figs/SNIa_lim22mag.pdf}};
\end{tikzpicture}
